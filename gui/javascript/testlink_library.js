...
// Add alert message that shows up whenever a page loads
// This will be executed when the document is fully loaded
document.addEventListener('DOMContentLoaded', function() {
  // alert('Welcome to TestLink - CBS Test Management System');
  
  // Set the background color of header-wrap to light grey
  var headerWrap = document.getElementById('header-wrap');
  if (headerWrap) {
    headerWrap.style.backgroundColor = '#e0e0e0'; // Light grey color
  }
  
  // Check the addLinkToTL and addLinkToTLPrintView checkboxes by default
  var addLinkToTL = document.getElementById('addLinkToTL');
  if (addLinkToTL) {
    addLinkToTL.checked = true;
  }
  
  var addLinkToTLPrintView = document.getElementById('addLinkToTLPrintView');
  if (addLinkToTLPrintView) {
    addLinkToTLPrintView.checked = true;
  }
});
...
{
  // Ensure tcversion_id is a valid value to prevent JavaScript errors
  tcversion_id = (tcversion_id !== undefined && tcversion_id !== null && tcversion_id !== '') ? tcversion_id : 0;
...
function dialog_onSubmit(odialog) {
  // Allow the form to submit normally without any special handling
  // This is important to prevent delays in form submission
  return true;
}
