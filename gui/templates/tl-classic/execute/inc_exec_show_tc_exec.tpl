...
{foreach item=tc_exec from=$gui->map_last_exec}
  ...
  <div id="execution_history" class="exec_history">
    ...
    p.on({'expand' : 
    function()
    {load_notes(this,{/literal}{$abs_last_exec.execution_id});}
    });
    };
      ...
      {if $gui->other_execs.$tcversion_id}
        ...
        {foreach item=tc_old_exec from=$gui->other_execs.$tcversion_id}
          ...
          <tr style="border-top:1px solid black; background-color: {$bg_color}">
            <td>
              ...
              {if $can_edit_exec_notes && $tc_old_exec.build_is_open}
                <img src="{$tlImages.note_edit}" style="vertical-align:middle" title="{$labels.edit_execution}" onclick="javascript: openExecEditWindow(
  		           {$tc_old_exec.execution_id},{$tc_old_exec.id},{$gui->tplan_id},{$gui->tproject_id});">
              {else}
                {if $can_edit_exec_notes}
                  <img src="{$tlImages.note_edit_greyed}" style="vertical-align:middle" title="{$labels.closed_build}">
                {/if}
              {/if}
          ...
          <td align="center"><a href="javascript:openFileUploadWindow({$tc_old_exec.execution_id},'executions')">
              <img src="{$tlImages.upload}" title="{$labels.alt_attachment_mgmt}" alt="{$labels.alt_attachment_mgmt}"
                style="border:none" /></a>
          </td>
        {else}
          {if $attachment_model->show_upload_column && $can_manage_attachments}
          ...
          <td align="center">
            {if $tc_old_exec.build_is_open}
              ...
              {if $gui->tlCanCreateIssue}
                <a
                  href="javascript:open_bug_add_window({$gui->tproject_id},{$gui->tplan_id},{$tc_old_exec.id},{$tc_old_exec.execution_id},0,'create')">
                  <img src="{$tlImages.bug_create_into_bts}" title="{$labels.bug_create_into_bts}" style="border:none" /></a>
              {/if}
            {else}
              <img src="{$tlImages.bug_link_tl_to_bts_disabled}" title="{$labels.bug_link_tl_to_bts}"
                style="border:none" /></a>
              &nbsp;&nbsp;
              {if $gui->tlCanCreateIssue}
                <img src="{$tlImages.bug_create_into_bts_disabled}" title="{$labels.bug_create_into_bts}"
                  style="border:none" /></a>
              {/if}
            {/if}
          </td>
        ...
        {if $can_delete_exec && $tc_old_exec.build_is_open}
          <td align="center">
            <a href="javascript:confirm_and_submit(msg,'execSetResults','exec_to_delete',
              ...
              <img src="{$tlImages.delete}" title="{$labels.img_title_delete_execution}" style="border:none" /></a>
          </td>
        {else}
          {if $can_delete_exec}
            <td align="center">
        ...
        {if $tlCfg->exec_cfg->steps_exec }
          <td class="icon_cell" align="center">
            <img src="{$tlImages.steps}" title="{$labels.access_test_steps_exec}" onclick="javascript:openPrintPreview('exec',{$tc_old_exec.execution_id},
        ...
        <script>
          {* Initialize panel if notes exists. There might be multiple note panels
            ...
            var p = new Ext.Panel({
              ...
              {load_notes(this,{/literal}{$tc_old_exec.execution_id});}
              });
              };
              panel_init_functions.push(panel_init);
            </script>
  ...
  <div class="{$theClass}">
    {if '' !== $hasNewestVersionMsg}
      ...
      {if $gui->hasNewestVersion}
        <div style="text-align: center;">
          <input type="hidden" id="TCVToUpdate" name="TCVToUpdate" value="{$gui->tcversionSet}">
    ...
    {$labels.title_test_case}&nbsp;{$gui->tcasePrefix|escape}{$cfg->testcase_cfg->glue_character}{$tc_exec.tc_external_id|escape}
    :: {$labels.version}: {$tc_exec.version} :: {$tc_exec.name|escape}
    <br />
  ...
  <div>
    ...
    {if $tc_exec.active eq 0}
      <h1 class="title">
        <center>{$labels.testcase_version_is_inactive_on_exec}</center>
      </h1>
    {/if}
    <hr />
  </div>
