...
{* Modified to always show execution details regardless of build selection *}
...
<script language="JavaScript" src="gui/javascript/execSetResults.js?v={$smarty.now}&cachebust=20260303"
  type="text/javascript"></script>
<script language="JavaScript" src="gui/niftycube/niftycube.js?v={$smarty.now}" type="text/javascript"></script>
<link rel="stylesheet" type="text/css" href="gui/niftycube/niftyCorners.css?v={$smarty.now}" />
<script language="JavaScript" type="text/javascript">
  var msg="{$labels.warning_delete_execution}";
  var import_xml_results="{$labels.import_xml_results}";
  window.currentTProjectId = {$gui->tproject_id|intval};
...
<body onLoad="show_hide('tplan_notes','{$tplan_notes_view_memory_id}',{$gui->tpn_view_status});
              ...
              {/if}" onUnload="storeWindowSize('TCExecPopup')">

  {* Apply Nifty rounded corners after page loads *}
  {if $round_enabled || #ROUND_TC_SPEC# || #ROUND_EXEC_HISTORY# || #ROUND_TC_TITLE#}
  ...
  <div id="main_content" class="workBack">
    ...
    <form method="post" id="execSetResults" name="execSetResults" enctype="multipart/form-data"
      ...
      {if $singleExec}
        <div class="groupBtn">
          ...
          <div class="direct_link" style='display:none'>
            <img class="clip" src="{$tlImages.clipboard}" title="eye" data-clipboard-text="{$gui->direct_link}">
          ...
          <input type="button" id="toggle_history_on_off" name="{$gui->history_status_btn_name}"
            value="{lang_get s=$gui->history_status_btn_name}" onclick="javascript:toogleRequiredOnShowHide('bug_summary');
          ...
          {if $gui->grants->execute}
            <input type="button" id="pop_up_import_button" name="import_xml_button" value="{$labels.import_xml_results}"
      ...
      <div id="{$div_id}" class="exec_additional_info">
        ...
        {if $gui->testplan_cfields neq ''} <div id="cfields_testplan" class="custom_field_container">
          {$gui->testplan_cfields}</div>{/if}
      </div>
      {* ---------------------------------------------------------------- *}
      ...
      <div id="{$div_id}" class="exec_additional_info">
        ...
        {if $gui->build_cfields != ''} <div id="cfields_build" class="custom_field_container">{$gui->build_cfields}
        </div>{/if}
      </div>
