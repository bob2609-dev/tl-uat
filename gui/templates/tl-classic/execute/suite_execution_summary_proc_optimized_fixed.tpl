...
{literal}
// Optimized Suite Execution Summary with TestLink-compatible UX
(function() {
    'use strict';
    ...
    function runReport() {
        ...
        // Use the AJAX endpoint for better performance
        $.ajax({
            url: '../../lib/execute/suite_execution_summary_ajax.php?action=run_report',
            type: 'POST',
            data: formData,
            dataType: 'json',
    ...
    function updatePlans() {
        ...
        $.ajax({
            url: '../../lib/execute/suite_execution_summary_ajax.php?action=get_plans',
            type: 'POST',
            ...
            success: function(response) {
                ...
                if (response.success && response.plans && response.plans.length > 0) {
                    ...
                    showNotification('Error loading test plans: ' + response.error, 'error');
                } else {
    ...
    function updateBuilds() {
        ...
        $.ajax({
            url: '../../lib/execute/suite_execution_summary_ajax.php?action=get_builds',
            type: 'POST',
            data: { testplan_id: planId },
            dataType: 'json',
            success: function(response) {
                const $select = $('#build_id');
                $select.html('<option value="">Select Build</option>');
                
                if (response.success && response.builds && response.builds.length > 0) {
                    response.builds.forEach(function(build) {
                        $select.append('<option value="' + build.id + '">' + build.name + '</option>');
                    });
                } else if (response.error) {
                    console.error('Error loading builds:', response.error);
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error loading builds:', status, error);
            }
        });
    }
