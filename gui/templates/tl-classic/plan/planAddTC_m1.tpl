...
<script type="text/javascript" src="{$basehref}gui/javascript/shift_select.js"></script>
	    ...
	    {if $gui->has_tc}
    	  ...
    	  <div class="groupBtn">
    			<div style="float: left; margin-right: 2em">
    				...
    				{if $gui->full_control}
      				<button
               onclick="cs_all_checkbox_in_div_with_platform('addTcForm', '{$add_cb}', Ext.get('select_platform').getValue()); return false">{$labels.adding_tc}</button>
    				{/if}
          ...
          <div style="margin: 10px; font-size: smaller; border: thin solid; padding:3px">
            {$gui->status_feedback|escape}
            {if $gui->keywords_filter_feedback != '' }
                <br/>{$labels.note_keyword_filter}: {$gui->keywords_filter_feedback|escape}
            {/if}
            {if $gui->platforms_filter_feedback != '' }
                <br/>{$labels.note_platform_filter}: {$gui->platforms_filter_feedback|escape}
            {/if}
          </div>  
    	  </div>
      {else}
