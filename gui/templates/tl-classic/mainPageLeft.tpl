...
{$bugsView="lib/execute/bugs_view.php"}
{$excelImportView="lib/admin/excelImport.php"}
  ...
  {if $display_left_block_2}
    <div class="list-group" style="{$divStyle}">
      ...
      {if $gui->grants.issuetracker_management || $gui->grants.issuetracker_view}
        <a href="{$issueTrackerView}{$gui->testprojectID}" class="list-group-item"
          style="{$aStyle}">{$labels.href_issuetracker_management}</a>
      {/if}
      ...
      <!--   <a href="{$bugsView}" class="list-group-item" style="{$aStyle}">Execution Bugs</a> -->
      <!-- COMMENTED -->
      <!--    {if $gui->grants.testplan_execute || $gui->grants.testplan_metrics}
      <a href="{$bugsView}" class="list-group-item" style="{$aStyle}">Execution Bugs</a>
    {/if} -->
    </div>
  {/if}
  ...
  {if $display_left_block_1}
    <div class="list-group" style="{$divStyle}">
      ...
      {if $gui->grants.tproject_user_role_assignment == "yes"}
        <a href="{$usersAssign}{$gui->testprojectID}" class="list-group-item"
          style="{$aStyle}">{$labels.href_assign_user_roles}</a>
      {/if}
      ...
      {if $gui->grants.keywords_view == "yes"}
        <a href="{$keywordsAssignment}{$gui->testprojectID}" class="list-group-item"
          style="{$aStyle}">{$labels.href_keywords_manage}</a>
      {/if}

      {if $gui->grants.platform_management || $gui->grants.platform_view}
        <a href="{$platformsView}{$gui->testprojectID}" class="list-group-item"
          style="{$aStyle}">{$labels.href_platform_management}</a>
      {/if}
      ...
      {/if}
      <a href="{$bugsView}" class="list-group-item" style="{$aStyle}">Execution Bugs</a>
      <a href="lib/execute/custom_bugtrack_integration.html" class="list-group-item" style="{$aStyle}">
      ...
      {if $gui->grants.project_edit == "yes"}
        ...
        {* Excel Import menu item - always visible *}
        <a href="{$excelImportView}" class="list-group-item" style="{$aStyle}">{$labels.href_excel_import}</a>
  ...
  {if $display_left_block_3}
    <div class="list-group" style="{$divStyle}">
      {if $gui->grants.reqs_view == "yes" || $gui->grants.reqs_edit == "yes" }
        ...
        <a href="{$gui->launcher}?feature=printReqSpec" class="list-group-item"
          style="{$aStyle}">{$labels.href_print_req}</a>
        <a href="{$gui->launcher}?feature=searchReq" class="list-group-item" style="{$aStyle}">{$labels.href_search_req}</a>
        <a href="{$gui->launcher}?feature=searchReqSpec" class="list-group-item"
          style="{$aStyle}">{$labels.href_search_req_spec}</a>
      {/if}
      ...
      {if $gui->grants.monitor_req == "yes"}
        <a href="{$reqMonOverView}{$gui->testprojectID}" class="list-group-item"
          style="{$aStyle}">{$labels.href_req_monitor_overview}</a>
      {/if}
    </div>
  {/if}
    ...
    <div class="list-group" style="{$divStyle}">
      ...
      {if $gui->hasKeywords}
        {if $gui->grants.keyword_assignment == "yes"}
          <a href="{$gui->launcher}?feature=keywordsAssign" class="list-group-item"
            style="{$aStyle}">{$labels.href_keywords_assign}</a>
        {/if}
      {/if}

      {if $gui->grants.modify_tc eq "yes"}
        <a href="{$tcCreatedUser}{$gui->testprojectID}" class="list-group-item"
          style="{$aStyle}">{$labels.link_report_test_cases_created_per_user}</a>
      {/if}
