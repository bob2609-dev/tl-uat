...
Submitted by: ' . $userName;
        ...
        function addIssue($summary, $description) {
            ...
            try {
                ...
                // Format the description according to the specified format with hardcoded values
                // This ensures the information appears in the issue description as required
                $formattedDescription = "BODY\n... (Tester Notes + issue details)\n--------------------------------\n";
                
                // Append the template format for user execution notes / issue description
                $formattedDescription .= "\n<user execution notes / Issue Description>\n=========================\nTest Data\n=========================\n";
                $formattedDescription .= "Test Scenario/Function ID:: " . $scenarioId . "\n";
                $formattedDescription .= "Action ID/Sub-scenario:: " . $subScenario . "\n";
                $formattedDescription .= "Test Details:: " . $testData . "\n";
                $formattedDescription .= "Expected Test Results:: " . $expectedResults . "\n";
                $formattedDescription .= "Error Detail:: " . $notes . "\n\n";
                
                $data = array(
                    'issue' => array(
                        'project_id' => $this->projectId,
                        'subject' => $cleanSummary,
                        'description' => $description . "\n\nSubmitted by: {$userName}",  // Also add to description as backup
                        'tracker_id' => 3, // Changed from Bug (1) to Feature (2), (3) Support
                        'priority_id' => 2, // Normal priority
                        'assigned_to_id' => 2635, // Hardcoded assignee ID
                        'custom_fields' => array(
                            array(
                                'id' => 16, // Using Reminders # field since we know it exists
                                'value' => $userName
                            )
                        )
                    )
                ...
                // Log that we're adding the assigned_to_id parameter
                redmine_log('Adding assigned_to_id: 2635 to the request');
