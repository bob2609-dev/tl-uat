...
def main(args=None):
    ...
    custom_field_map = {
        ...
        'case_priority': 15,  # "Case_Priority" (moved before bug_ids)
        'bug_ids': 14,        # "BUG_IDS" (moved after case_priority)
    }
    ...
    expected_columns = [
        ...
        'Case_Priority',                                              # case_priority (11th column)
        'BUG_IDS'                                                     # bug_ids (12th column)
    ]
    ...
    column_map = {
        ...
        'case_priority': df.columns[10] if num_columns > 10 else None,  # Eleventh column (column K)
        'bug_ids': df.columns[11] if num_columns > 11 else None,        # Twelfth column (column L)
    }
    
    # Print column mapping for easier debugging
    print("\nDetailed Excel columns:")
    ...
    with open(config['output_sql'], 'w', encoding='utf-8') as sql_file:
        ...
        for index, row in df.iterrows():
            ...
            case_priority = get_column_value('case_priority')
            bug_ids = get_column_value('bug_ids')
...
""")
            ...
            # Only insert case_priority if it's not empty and not TEMPLATE_EMPTY
            if case_priority and case_priority != 'TEMPLATE_EMPTY':
                sql_file.write(f"INSERT INTO `cfield_design_values` (`field_id`, `node_id`, `value`) "
                               f"VALUES ({custom_field_map['case_priority']}, @tcversion_id, '{case_priority}');\n")
                
            # Only insert bug_ids if it's not empty and not TEMPLATE_EMPTY
            if bug_ids and bug_ids != 'TEMPLATE_EMPTY':
                sql_file.write(f"INSERT INTO `cfield_design_values` (`field_id`, `node_id`, `value`) "
                               f"VALUES ({custom_field_map['bug_ids']}, @tcversion_id, '{bug_ids}');\n")
