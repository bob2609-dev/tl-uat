...
-- This query extracts test case data in a format similar to C2B1_LENDING_OD.xlsx
-- Generated based on CHEQUE CASA_20250724222505.sql structure and import logic

SELECT 
    -- Module Name (from custom field 1)
    COALESCE(cf1.value, '') AS 'Module Name',
    
    -- Scenario ID / Function ScreenID Primary Module/ Function ID (from custom field 2)
    COALESCE(cf2.value, '') AS 'Scenario ID / Function ScreenID Primary Module/ Function ID',
    
    -- Sub-Scenario/Action (from custom field 3)
    COALESCE(cf3.value, '') AS 'Sub-Scenarion/Action (e.g. New, Modify, Close)',
    
    -- Test Case S. No. (from nodes_hierarchy name)
    nh_tc.name AS 'Test Case S. No.',
    
    -- Test Case Description (from custom field 6 or test script)
    COALESCE(cf6.value, tcv.summary) AS 'Test Case Description',
    
    -- Test Type (-ve/+ve) (from custom field 5)
    COALESCE(cf5.value, '') AS 'Test Type (-ve/+ve)',
    
    -- Test script (from custom field 6 or summary)
    COALESCE(cf6.value, REPLACE(REPLACE(tcv.summary, '<p>', ''), '</p>', '')) AS 'Test script',
    
    -- Test Execution path (from custom field 7)
    ...
    -- Bug IDs (comma-separated from vw_execution_bugs)
