...
if (isset($_GET['execution_id'])) {
    ...
    // TEST: Verify the file was loaded
    if (function_exists('getCustomIssueData')) {
        file_put_contents(
            'redmine_status_api.log',
            date('[Y-m-d H:i:s] ') . "CUSTOM INTEGRATION FILE LOADED SUCCESSFULLY\n",
            FILE_APPEND
        );
    } else {
        file_put_contents(
            'redmine_status_api.log',
            date('[Y-m-d H:i:s] ') . "ERROR: CUSTOM INTEGRATION FILE NOT LOADED\n",
            FILE_APPEND
        );
    }
        ...
        if ($result) {
            while ($row = $db->fetch_array($result)) {
                ...
                if ($bug_data) {
                    ...
                    file_put_contents(
                        'redmine_status_api.log',
                        date('[Y-m-d H:i:s] ') . "Bug data: " . json_encode($bug_data) . "\n",
                        FILE_APPEND
                    );
...
// Initialize database connection for custom integration
require_once('config.inc.php');
require_once('common.php');
require_once('lib/execute/custom_issue_integration_safe.php');
$db = new database(DB_TYPE);
...
$bug_data = getCustomIssueData($db, $tproject_id, $bugID);
...
if ($bug_data) {
    echo json_encode([
        ...
        'bug_id' => $bugID,
        'bug_data' => $bug_data,
        'debug' => 'Successfully fetched bug using custom integration',
        'debug_info' => array(
            'tproject_id' => $tproject_id,
            'using_custom_integration' => true
        )
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Failed to fetch bug data from both custom integration and fallback',
        'bug_id' => $bugID,
        'bug_data' => null,
        'debug' => 'Both custom integration and fallback failed',
        'debug_info' => array(
            'tproject_id' => $tproject_id,
            'using_custom_integration' => true
        )
