...
else 
{
  ...
  if($op['status_ok'])
  {
    ...
    if(($op['status_ok'] = !is_null($tproject_data))) 
    {
      ...
      $safeItem = htmlspecialchars((string)$args->item, ENT_QUOTES, 'UTF-8');
      $op['msg'] = sprintf(lang_get('invalid_item'), $safeItem);
    }
    else 
    {
      $safePrefix = htmlspecialchars((string)$args->tprojectPrefix, ENT_QUOTES, 'UTF-8');
      $op['msg'] = sprintf(lang_get('testproject_not_found'), $safePrefix);
    }
  } 
...
{
  $args = new stdClass();
  $args->tprojectPrefix = isset($_GET['tprojectPrefix']) ? (string)$_GET['tprojectPrefix'] : null;
  $args->id = isset($_GET['id']) ? (string)$_GET['id'] : null;

  // Sanitize anchor: allow only simple fragment identifiers and normalize to leading '#'
  $rawAnchor = isset($_GET['anchor']) ? (string)$_GET['anchor'] : null;
  if ($rawAnchor !== null) {
    $clean = preg_replace('/[^A-Za-z0-9_\-:.]/', '', $rawAnchor);
    $args->anchor = $clean !== '' ? ('#' . $clean) : '';
  } else {
    $args->anchor = '';
  }

  // version can be numeric or simple string; keep as string but trim
  $args->version = isset($_GET['version']) ? (string)$_GET['version'] : null;
  // Whitelist item values
  $allowedItems = array('req','reqspec','testcase','testsuite');
  $item = isset($_GET['item']) ? (string)$_GET['item'] : null;
  $args->item = in_array($item, $allowedItems, true) ? $item : null;
...
function buildLink(&$argsObj)
{
  ...
  $lk = $argsObj->item ? ("item=" . rawurlencode($argsObj->item)) : '';
  $lk .= $argsObj->id ? ("&id=" . rawurlencode($argsObj->id)) : '';
  $lk .= $argsObj->version ? ("&version=" . rawurlencode($argsObj->version)) : '';
  $lk .= $argsObj->tprojectPrefix ? ("&tprojectPrefix=" . rawurlencode($argsObj->tprojectPrefix)) : '';
  // Pass anchor as param for inner load; encode value
  $anchorParam = '';
  if (isset($_GET['anchor'])) {
    $anchorParam = '&anchor=' . rawurlencode((string)$_GET['anchor']);
  }
  $lk .= '&load' . $anchorParam;
