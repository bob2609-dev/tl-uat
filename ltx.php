...
else 
{
  ...
  $driver = isset($_GET['item']) ? (string)$_GET['item'] : null;
  $allowedDrivers = array('exec','xta2m');
  if(is_null($driver) || !in_array($driver,$allowedDrivers,true))
  {
    die();
  }  
...
function init_args(&$dbHandler)
{
  ...
  // Sanitize anchor: allow only safe fragment chars (no spaces or angle brackets)
  if (isset($_GET['anchor'])) {
    $rawAnchor = (string)$_GET['anchor'];
    $args->anchor = preg_replace('/[^A-Za-z0-9_\-:.]/','',$rawAnchor);
  } else {
    $args->anchor = '';
  }

  // Whitelist item
  $allowedItems = array('exec','xta2m');
  $item = isset($_GET['item']) ? (string)$_GET['item'] : null;
  $args->item = in_array($item,$allowedItems,true) ? $item : null;

  // Numeric feature id
  $args->feature_id = intval(isset($_GET['feature_id']) ? $_GET['feature_id'] : null);
...
function build_link_exec(&$argsObj)
{
  ...
  {
    $lk .= "&feature_id=" . rawurlencode((string)$argsObj->feature_id);
  } 
  else
  {
    $lk .= "&tplan_id=" . rawurlencode((string)$argsObj->tplan_id);
    $lk .= "&platform_id=" . rawurlencode((string)$argsObj->platform_id);
    $lk .= "&tcversion_id=" . rawurlencode((string)$argsObj->tcversion_id);
  } 
  $lk .= "&build_id=" . rawurlencode((string)$argsObj->build_id);
  $anchorParam = $argsObj->anchor !== '' ? ('&anchor=' . rawurlencode($argsObj->anchor)) : '';
  $lk .= '&load' . $anchorParam;
