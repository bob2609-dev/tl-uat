...
function doUpdate(&$dbHandler,&$argsObj,$sessionUserID)
{
  ...
  if ($op->status >= tl::OK)
  {
    // Harden role changes against client-side tampering (role escalation via DevTools)
    // 1) Preserve existing role by default
    $targetCurrentRoleId = $op->user->globalRoleID;
    $requestedRoleId = isset($argsObj->rights_id) ? intval($argsObj->rights_id) : $targetCurrentRoleId;

    // 2) Validate requested role exists
    $allRoles = tlRole::getAll($dbHandler,null,null,null,tlRole::TLOBJ_O_GET_DETAIL_MINIMUM);
    $validRoleIds = array_keys((array)$allRoles);
    $isRequestedRoleValid = in_array($requestedRoleId, $validRoleIds, true);

    // 3) If non-admin attempts to assign Admin role, deny and keep existing
    $sessionUser = isset($_SESSION['currentUser']) ? $_SESSION['currentUser'] : null;
    $sessionIsAdmin = ($sessionUser && intval($sessionUser->globalRoleID) === TL_ROLES_ADMIN);
    if (!$isRequestedRoleValid || (!$sessionIsAdmin && intval($requestedRoleId) === TL_ROLES_ADMIN)) {
      // Reject role change and keep existing role
      $argsObj->rights_id = $targetCurrentRoleId;
    } else {
      $argsObj->rights_id = $requestedRoleId;
    }
...
function createNewPassword(&$dbHandler,&$argsObj,&$userObj,$newPasswordSendMethod)
{
  ...
  {
    // Ensure array shape regardless of implementation details
    $dummy = (array) resetPassword($dbHandler,$argsObj->user_id,$newPasswordSendMethod);
