...
function tlSetCookie($ckObj) {
  ...
  // Determine HTTPS to set 'secure' flag safely
  $isHttps = (
  ...
  // Build options, defaulting to secure/httponly and Lax SameSite
  $sameSite = property_exists($stdCk, 'samesite') && !empty($stdCk->samesite) ? $stdCk->samesite : 'Lax';
  $opts = array(
    'expires'  => isset($stdCk->expire) ? intval($stdCk->expire) : 0,
    'path'     => isset($stdCk->path) ? $stdCk->path : '/',
    'domain'   => isset($stdCk->domain) ? $stdCk->domain : '',
    'secure'   => $isHttps || !empty($stdCk->secure),
    'httponly' => property_exists($stdCk, 'httponly') ? (bool)$stdCk->httponly : true,
    'samesite' => $sameSite
  );
  // Diagnostic logging for cookie setup
  if (function_exists('tLog')) {
    tLog("tlSetCookie name={$stdCk->name} domain={$opts['domain']} path={$opts['path']} secure=" .
         (($opts['secure']) ? '1' : '0') . " httponly=" . (($opts['httponly']) ? '1' : '0') .
         " samesite={$opts['samesite']} expires={$opts['expires']} isHttps=" . (($isHttps) ? '1' : '0'), 'INFO');
  }

  if (PHP_VERSION_ID >= 70300) {
    setcookie($stdCk->name, $stdCk->value, $opts);
  } else {
    // PHP 7.2 and below: build Set-Cookie header manually to include SameSite
    $cookieParts = array();
    $cookieParts[] = rawurlencode($stdCk->name) . '=' . rawurlencode((string)$stdCk->value);
    if (!empty($opts['expires'])) {
      $cookieParts[] = 'Expires=' . gmdate('D, d-M-Y H:i:s T', intval($opts['expires']));
      $cookieParts[] = 'Max-Age=' . max(0, intval($opts['expires']) - time());
    }
    $cookieParts[] = 'Path=' . $opts['path'];
    if (!empty($opts['domain'])) {
      $cookieParts[] = 'Domain=' . $opts['domain'];
    }
    ...
    if (!empty($opts['samesite'])) {
      $cookieParts[] = 'SameSite=' . $opts['samesite'];
    }
    @header('Set-Cookie: ' . implode('; ', $cookieParts), false);
  }

  if (function_exists('tLog')) {
    tLog("tlSetCookie post-set headers_sent=" . (headers_sent() ? '1' : '0') .
         " cookie_present_on_superglobal=" . (isset($_COOKIE[$stdCk->name]) ? '1' : '0'), 'INFO');
  }
}
