...
class searchCommands
{
  ...
  private $views;
  private $likeOp;
  private $filterParams;  // holds bound values for dynamic filters (e.g., dates)
  ...
  /**
   * Accessor for filter params accumulated during initArgs()
   */
  ...
  function initArgs()
  {
    ...
    // cap target length to mitigate abuse
    ...
    $this->filterParams = array('dates4tc' => array(), 'dates4rq' => array());
    foreach($k2w as $key => $value)
    {
      ...
      if (isset($args->$key) && $args->$key != '') 
      {
        ...
        if ($da != null) 
        {
          ...
          // Use placeholders and collect params for later merge
          $this->filters['dates4tc'][$key] = " AND TCV.{$k2f[$key]} ? ";
          $this->filterParams['dates4tc'][] = $args->$key;

          $this->filters['dates4rq'][$key] = " AND RQV.{$k2f[$key]} ? ";
          $this->filterParams['dates4rq'][] = $args->$key;
  ...
  function searchReqSpec($targetSet,$canUseTarget) {
    ...
    $sql = "SELECT RSRV.name, RSRV.scope, LRSR.req_spec_id, RSRV.id, LRSR.revision " .
           ...
           "WHERE LRSR.testproject_id = ?";
    $params = array(intval($args->tproject_id));
    ...
    if( $canUseTarget ) {
      $doFilter = true;
      $conj = (strtolower($args->and_or) === 'and') ? 'AND' : 'OR';

      // Scope filter group
      $scopeConds = array();
      foreach($targetSet as $target) {
        $scopeConds[] = ($udf ? "$udf(RSRV.scope)" : "RSRV.scope") . " $this->likeOp ?";
        $params[] = $db->likeParamContains($target);
      }
      if(count($scopeConds) > 0) {
        $filterRS['scope'] = ' ( ' . implode(" $conj ", $scopeConds) . ' ) ';
      }
      ...
      foreach($targetSet as $trgt) {
        $target = trim($trgt);
        $nameConds[] = "RSRV.name $this->likeOp ?";
        $params[] = $db->likeParamContains($target);
      }
      if(count($nameConds) > 0) {
        $filterRS['name'] = ' ( ' . implode(" $conj ", $nameConds) . ' ) ';
      }
    }  

    $otherFRS = '';  
    if(!is_null($filterRS)) {
      $otherFRS = " AND (" . implode(" OR ", $filterRS) . ")";
    }  

    $sql .= $otherFRS;
    if($doFilter) {
      $mapRSpec = $db->fetchRowsIntoMapParams($sql,'req_spec_id',$params); 
    }  
    return $mapRSpec;
  } 
  ...
  function searchReq($targetSet,$canUseTarget,$req_cf_id) {
    ...
    {      
      ...
      switch($gui->cf_types[$cf_def['type']])
      {
        case 'date':
          ...
          $fi['by_custom_field'] .= " AND CFD.value = ?";
          $params[] = intval($args->custom_field_value);
        break;

        default:
          $fi['by_custom_field'] .= " AND CFD.value $this->likeOp ? ";
          $params[] = $db->likeParamContains($args->custom_field_value);
        break;
      }
    }  
    ...
    if($args->created_by != '' )
    {
      ...
      $fi['author'] = " AND ( RQAUTHOR.login $this->likeOp ? OR " . 
                          "       RQAUTHOR.first $this->likeOp ? OR " . 
                          "       RQAUTHOR.last $this->likeOp ? ) ";
      $pat = $db->likeParamContains($args->created_by);
      $params[] = $pat; $params[] = $pat; $params[] = $pat;
    }  
    ...
    if( $args->edited_by != '' )
    {
      ...
      $fi['modifier'] = " AND ( UPDATER.login $this->likeOp ? OR " .
                            "       UPDATER.first $this->likeOp ? OR " .
                            "       UPDATER.last $this->likeOp ? ) ";
      $pat = $db->likeParamContains($args->edited_by);
      $params[] = $pat; $params[] = $pat; $params[] = $pat;
    }  
    ...
    if( $doSql ) {  
      ...
      if( $canUseTarget ) {
        ...
        if( $args->rq_scope ) {
          $scopeConds = array();
          foreach($targetSet as $target) {
            $scopeConds[] = ($udf ? "$udf(RQV.scope)" : "RQV.scope") . " $this->likeOp ?";
            $params[] = $db->likeParamContains($target);
          }
          if(count($scopeConds) > 0) {
            $filterRQ['scope'] = ' ( ' . implode(" $conj ", $scopeConds) . ' ) ';
          }
        }  
        ...
        if( $args->rq_title )
        {
          ...
          foreach($targetSet as $target)
          {
            $nameConds[] = " NHRQ.name $this->likeOp ? "; 
            $params[] = $db->likeParamContains($target);
          }  
          if(count($nameConds) > 0) {
            $filterRQ['name'] = ' ( ' . implode(" $conj ", $nameConds) . ' ) ';
          }
        }  
        ...
        if( $args->rq_doc_id )
        {
          ...
          foreach($targetSet as $target)
          {
            $docConds[] = " RQ.req_doc_id $this->likeOp ? ";  
            $params[] = $db->likeParamContains($target);
          }  
          if(count($docConds) > 0) {
            $filterRQ['req_doc_id'] = ' ( ' . implode(" $conj ", $docConds) . ' ) ';
          }
        } 
      } 
      ...
      if(!is_null($filterRQ))
      {
        $otherFRQ = " AND (" . implode(" OR ", $filterRQ) . ")";
      }  
      ...
      // append parameterized date filters for requirements
      $mixedFilter = $this->getFilters();
      $filterParams = $this->getFilterParams();
      if ($mixedFilter && isset($mixedFilter['dates4rq']) && $mixedFilter['dates4rq']) {
        $sql .= implode("", $mixedFilter['dates4rq']);
        if (isset($filterParams['dates4rq']) && $filterParams['dates4rq']) {
          $params = array_merge($params, $filterParams['dates4rq']);
        }
      }
      if( $doFilter ) 
      {
        //DEBUGecho __FUNCTION__ . ' SQL Line:' . __LINE__ . $sql .'<br>';
        $mapRQ = $db->fetchRowsIntoMapParams($sql,'req_id',$params); 
      }
  ...
  function searchTestSuites($targetSet,$canUseTarget) {
    ...
    if( ($doIt = $args->ts_summary && $canUseTarget) ) {
      $sumConds = array();
      foreach($targetSet as $target) {
        $sumConds[] = ($udf ? "$udf(TS.details)" : "TS.details") . " $this->likeOp ?";
        $params[] = $db->likeParamContains($target);
      }
      if(count($sumConds) > 0) {
        $filterSpecial['ts_summary'] = ' ( ' . implode(" $conj ", $sumConds) . ' ) ';
      }
    }  
    ...
    if( ($doIt = $args->ts_title && $canUseTarget) ) {
      $titleConds = array();
      foreach($targetSet as $target) {
        $titleConds[] = " NH_TS.name $this->likeOp ? ";
        $params[] = $db->likeParamContains($target);
      }  
      if(count($titleConds) > 0) {
        $filterSpecial['ts_title'] = ' ( ' . implode(" $conj ", $titleConds) . ' ) ';
      }
    }  
    ...
    {
      $otherFilters = " AND (" . implode(" OR ", $filterSpecial) . ")";
    }  
    ...
    if($args->ts_title || $args->ts_summary)
    {
      ...
      if($args->keyword_id)
      {
       ...
       $filterTS['by_keyword_id'] = " AND KW.keyword_id = ? ";
       $params[] = intval($args->keyword_id);
      }  
      ...
      $mapTS = $db->fetchRowsIntoMapParams($sql,'id',$params); 
  ...
  function searchTestCases($tcaseSet,$targetSet,$canUseTarget,$tc_cf_id) {
    ...
    // Build special filter groups to combine with OR between groups
    ...
    if($args->tc_id)
    {
      ...
      foreach($targetSet as $tgx)
      {
        ...
        {
          $idConds[] = " TCV.tc_external_id = ? ";
          $params[] = intval($target);
        }  
      }
      if(count($idConds) > 0) {
        $filterSpecial['by_tc_id'] = ' ( ' . implode(" $conj ", $idConds) . ' ) ';
      }
    }  
    ...
    if($tc_cf_id > 0)
    {
      ...
      switch($gui->cf_types[$cf_def['type']])
      {
        case 'date':
          ...
          $filter['by_custom_field'] .= " AND CFD.value = ?";
          $params[] = intval($args->custom_field_value);
        break;

        default:
          $filter['by_custom_field'] .= " AND CFD.value $this->likeOp ? ";
          $params[] = $db->likeParamContains($args->custom_field_value);
        break;
      }
    }
    ...
    if($args->tc_steps && $canUseTarget) {
      $stepConds = array();
      foreach($targetSet as $target) {
        $stepConds[] = ($udf ? "$udf(TCSTEPS.actions)" : "TCSTEPS.actions") . " $this->likeOp ?";  
        $params[] = $db->likeParamContains($target);
      }  
      if(count($stepConds) > 0) {
        $filterSpecial['by_steps'] = ' ( ' . implode(" $conj ", $stepConds) . ' ) ';
      }
    }    
    ...
    if($args->tc_expected_results && $canUseTarget) {
      $expConds = array();
      foreach($targetSet as $target) {
        $expConds[] = ($udf ? "$udf(TCSTEPS.expected_results)" : "TCSTEPS.expected_results") . " $this->likeOp ?"; 
        $params[] = $db->likeParamContains($target);
      }  
      if(count($expConds) > 0) {
        $filterSpecial['by_expected_results'] = ' ( ' . implode(" $conj ", $expConds) . ' ) ';
      }
    }    
      ...
      foreach($k2w as $kf => $alias)
      {
        ...
        if($args->$in)
        {
          ...
          foreach($targetSet as $target) {
            ...
            $textConds[] = " {$xx} {$this->likeOp} ? "; 
            $params[] = $db->likeParamContains($target);
          }  
          if(count($textConds) > 0) {
            $filterSpecial[$kf] = ' ( ' . implode(" $conj ", $textConds) . ' ) ';
          }
        }
      }     
    } 
    ...
    $otherFilters = '';  
    if(!is_null($filterSpecial) && count($filterSpecial) > 0)
    {
      $otherFilters = " AND (/* filterSpecial */ " . 
                      implode(" OR ", $filterSpecial) . ")";
    }  
    ...
    if($doFilter)
    {
      ...
      if( $created_by_on_tc != '' )
      {
        ...
        $filter['author'] = " AND ( AUTHOR.login $this->likeOp ? OR " .
                            "       AUTHOR.first $this->likeOp ? OR " .
                            "       AUTHOR.last $this->likeOp ? ) ";
        $pat = $db->likeParamContains($args->created_by);
        $params[] = $pat; $params[] = $pat; $params[] = $pat;
      }  
      ...
      if( $edited_by_on_tc != '' )
      {
        ...
        $filter['modifier'] = " AND ( UPDATER.login $this->likeOp ? OR " .
                            "         UPDATER.first $this->likeOp ? OR " .
                            "         UPDATER.last $this->likeOp ? ) ";
        $pat = $db->likeParamContains($args->edited_by);
        $params[] = $pat; $params[] = $pat; $params[] = $pat;
    ...
    if($doFilter)
    {
      ...
      if ($mixedFilter['dates4tc'])
      {
        ...
        if (isset($filterParams['dates4tc']) && $filterParams['dates4tc']) {
          $params = array_merge($params, $filterParams['dates4tc']);
        }
      }
      ...
      $mapTC = $db->fetchRowsIntoMapParams($sql,'testcase_id',$params); 
    }  
