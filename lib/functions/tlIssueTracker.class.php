...
class tlIssueTracker extends tlObject
{
  ...
  function getAll($options=null)
  {
    ...
    // Secure ORDER BY: allow only specific columns and directions
    $orderByClause = '';
    if( !is_null($my['options']['orderByField']) )
    {
      $raw = trim($my['options']['orderByField']);
      // Support formats like "name DESC" or "name:DESC"
      if(strpos($raw,':') !== false){
        list($candCol,$candDir) = array_pad(explode(':',$raw,2),2,null);
      } else {
        $parts = preg_split('/\s+/', $raw);
        $candCol = isset($parts[0]) ? $parts[0] : null;
        $candDir = isset($parts[1]) ? $parts[1] : null;
      }
      $allowedCols = array(
        'name' => 'name',
        'id'   => 'id',
      ...
      $col = $this->db->orderByColumn($candCol, $allowedCols, 'name');
      $dir = $this->db->orderByDirection($candDir, 'ASC');
