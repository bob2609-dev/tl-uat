...
function doSessionStart($setPaths=false) {
  ...
  if( PHP_SESSION_NONE == session_status() ) {
    // Enhance cookie security with Secure, HttpOnly and SameSite flags
    ...
    // PHP 7.3+ supports the options array format
    if (PHP_VERSION_ID >= 70300) {
      ...
      // For PHP < 7.3, we can't set SameSite directly, but we can set the other flags
      session_set_cookie_params(99999, '/; SameSite=Lax', '', $secure, true);
    }
  }
