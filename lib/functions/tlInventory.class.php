...
class tlInventory extends tlObjectWithDB
{
	 ...
	 * @param mixed $inputData array|object with machine fields
	 */
	protected function initInventoryData($inputData)
	{
		// Support both array and stdClass input
		if (is_array($inputData)) {
			$src = $inputData;
		} else if (is_object($inputData)) {
			// get_object_vars maintains public properties only
			$src = get_object_vars($inputData);
		} else {
			$src = array();
		}

		$this->inventoryId = isset($src['machineID']) ? intval($src['machineID']) : 0;
		$this->name = isset($src['machineName']) ? $src['machineName'] : '';
		$this->ipAddress = isset($src['machineIp']) ? $src['machineIp'] : '';
		$this->ownerId = isset($src['machineOwner']) ? intval($src['machineOwner']) : 0;
		$this->inventoryContent['notes'] = isset($src['machineNotes']) ? $src['machineNotes'] : '';
		$this->inventoryContent['purpose'] = isset($src['machinePurpose']) ? $src['machinePurpose'] : '';
		$this->inventoryContent['hardware'] = isset($src['machineHw']) ? $src['machineHw'] : '';
	}
	...
	protected function checkInventoryData()
	{
		...
		// Validate ownerId corresponds to an existing active user
		if ($result == tl::OK) {
			$ownerOk = false;
			$ownerId = intval($this->ownerId);
			if ($ownerId > 0) {
				$owner = new tlUser($ownerId);
				$st = $owner->readFromDB($this->db);
				$ownerOk = ($st >= tl::OK) && (intval($owner->isActive) === 1);
			}
			if (!$ownerOk) {
				$result = tl::ERROR;
				$this->userFeedback = lang_get('inventory_invalid_owner');
			}
		}
