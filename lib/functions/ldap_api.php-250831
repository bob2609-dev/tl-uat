 ...
 */
 ...
 // LDAP Extension Check - Add this at the top of the file
if (!extension_loaded('ldap')) {
    // Try to load the LDAP extension dynamically
    ...
    if (!extension_loaded('ldap')) {	
        ...
        echo "<h1>LDAP Extension Not Available</h1>";
        echo "<p>The PHP LDAP extension is required for LDAP authentication but is not loaded.</p>";
        echo "<p>Please enable the LDAP extension in your PHP configuration or contact your administrator.</p>";
        exit;
    }
...
function ldap_connect_bind( $authCfg, $p_binddn = '', $p_password = '') 
{
  ...
  {
    $ret->handler=$t_ds;
	ldap_set_option($t_ds, LDAP_OPT_NETWORK_TIMEOUT, isset($authCfg['ldap_timeout']) ? $authCfg['ldap_timeout'] : 5);
ldap_set_option($t_ds, LDAP_OPT_TIMELIMIT, isset($authCfg['ldap_timeout']) ? $authCfg['ldap_timeout'] : 5);
