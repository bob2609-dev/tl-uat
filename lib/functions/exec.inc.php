...
{
  // Add logging to track bug linking operations
  $timestamp = date('Y-m-d H:i:s') . '.' . substr(microtime(true) * 1000, -3);
  $logMessage = "[{$timestamp}] [DEBUG] [WRITE_EXEC_BUG] exec_id: {$exec_id}, bug_id: {$bug_id}, tcstep_id: {$tcstep_id}, just_delete: " . ($just_delete ? 'YES' : 'NO') . "\n";
  error_log($logMessage, 3, 'execution_debug.log');
  ...
  error_log("[{$timestamp}] [DEBUG] [WRITE_EXEC_BUG] DELETE SQL: " . $sql, 3, 'C:/xampp/htdocs/testlink/logs/execution_debug.log');
  $result = $db->exec_query($sql);
  error_log("[{$timestamp}] [DEBUG] [WRITE_EXEC_BUG] DELETE RESULT: " . ($result ? 'SUCCESS' : 'FAILED'), 3, 'C:/xampp/htdocs/testlink/logs/execution_debug.log');
  ...
  if(!$just_delete)
  {
    $sql = " INSERT INTO {$execution_bugs} (execution_id,tcstep_id,bug_id) " .
           ...
           " ,'" . $prep_bug_id . "')";
    error_log("[{$timestamp}] [DEBUG] [WRITE_EXEC_BUG] INSERT SQL: " . $sql, 3, 'C:/xampp/htdocs/testlink/logs/execution_debug.log');
    $result = $db->exec_query($sql);         
    error_log("[{$timestamp}] [DEBUG] [WRITE_EXEC_BUG] INSERT RESULT: " . ($result ? 'SUCCESS' : 'FAILED'), 3, 'C:/xampp/htdocs/testlink/logs/execution_debug.log');
  }
...
function generateIssueText($dbHandler,$argsObj,$itsObj,$opt=null) {
  ...
  // Only include timestamp in summary if not explicitly skipped
  if (property_exists($argsObj,'skipTimestamp') && $argsObj->skipTimestamp === true) {
    $ret->summary = $ret->auditSign;
  } else {
  ...
  if(property_exists($argsObj,'bug_summary') && strlen(trim($argsObj->bug_summary)) != 0 ) {
    ...
    $formattedSummary = $argsObj->bug_summary;
    ...
    // Remove any timestamp information from the bug summary
    $cleanedSummary = preg_replace('/ - Executed ON \(ISO FORMAT\): .*$/i', '', $argsObj->bug_summary);
    $argsObj->bug_summary = $cleanedSummary;
    
    if (preg_match('~Test Case: /([^/]+)/([^/]+)/(.+?)$~', $argsObj->bug_summary, $matches)) {
    ...
    elseif (preg_match('~/([^/]+)/([^/]+)/(.+?)$~', $argsObj->bug_summary, $matches)) {
      $projectName = $matches[1];
      $testcasePath = $matches[2] . '/' . $matches[3];
      // Replace slashes with greater-than symbols in the testcase path
      $formattedPath = str_replace('/', ' > ', $testcasePath);
