...
{
  // *******************************************************************************
  // PERFORMANCE OPTIMIZATION: Import global execution counter flag
  // *******************************************************************************
  // When set to TRUE, disables recursive calculation of passed/failed/blocked counters
  ...
  // Optimized Tree Generation Strategy (Bottom-Up)
  // 1. Fetch relevant test cases (leaves) first.
  ...
  // 2. Build Tree from Leaves (or fallback)
  ...
  $map_node_tccount = array();
    ...
    // *******************************************************************************
    // PERFORMANCE NOTE: This modification disables PHP recursion processing 
    // (iterating through 90,000+ nodes and summing integers).
    // This is significant for CPU usage reduction.
    // 
    // IMPORTANT: This does NOT disable the initial SQL query that fetches 
    // execution status for 90,000+ rows, as that data is still needed
    // for proper node coloring (Pass/Fail icons) in the execution tree.
    //
    // SIDE EFFECTS: When counters are disabled:
    // - "Hide Test Cases" filter option will stop working
    // - Empty test suite folders will not be pruned
    // - Tree will show only basic test case counts, not status breakdown
    //
    if (!$g_disable_execution_counters) {
      // Original recursive counter calculation (preserves existing behavior)
      ...
      // This provides basic test case counts without expensive recursion
    ...
    // *******************************************************************************
    // Apply global counter setting to render options
    // This ensures tree rendering respects the performance flag
    ...
    } // End of if (!is_null($tplan_tcases)) block
...
function applyStatusFilters($tplan_id, &$items2filter, &$fobj, &$tplan_mgr, $statusCfg)
{
  ...
  $ffn = array(
    $methods['any_build'] => 'filterStatusSetAtLeastOneOfActiveBuilds',
...
function testPlanTree(
  ...
  if ($test_spec) {
    ...
    if (isset($my['filters']['filter_custom_fields']) && isset($test_spec['childNodes'])) {
      $test_spec['childNodes'] = filter_by_cf_values(
        $dbHandler,
