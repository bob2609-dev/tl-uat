...
class database {
  ...
  /**
   * Execute a parameterized SQL query using ADODB prepared-execution with positional placeholders (?).
   * This avoids risky string concatenation and should be preferred for any user-influenced values.
   *
   * @param string $sql SQL with ? placeholders
   * @param array  $params Values to bind in order
   * @param int    $limit Optional row limit
   * @param int    $offset Optional row offset
   * @return mixed ADODB resultset|false
   */
  function exec_query_params($sql, $params = array(), $limit = -1, $offset = -1)
  {
    $ec = 0;
    $emsg = null;
    $logLevel = 'DEBUG';
    ...
    // ADODB supports params in both Execute($sql, $params) and SelectLimit($sql, $rows, $start, $params)
    if ( ( $limit != -1 ) || ( $offset != -1 ) ) {
      $t_result = $this->db->SelectLimit($sql, $limit, $offset, $params);
    } else {
      $t_result = $this->db->Execute($sql, $params);
    }
    ...
    // Append params in log (truncated for safety)
    if(!empty($params)){
      $safeParams = $params;
      foreach($safeParams as $k => $v){
        if(is_string($v) && strlen($v) > 200){ $safeParams[$k] = substr($v,0,200) . '...'; }
      }
    ...
    if (!$t_result)
    {
      ...
      tLog("ERROR ON exec_query_params() - database.class.php <br />" . $this->error(htmlspecialchars($sql)) . 
           "<br />THE MESSAGE : $message ", 'ERROR', "DATABASE");
      echo "<pre> ============================================================================== </pre>";
      echo "<pre> DB Access Error - debug_print_backtrace() OUTPUT START </pre>";
      echo "<pre> ATTENTION: Enabling more debug info will produce path disclosure weakness (CWE-200) </pre>";
    ...
    if($this->logQueries)
    {
      // keep query text; params are already included above in $message
      array_push ($this->queries_array, array( $sql, $t_elapsed, $ec, $emsg ) );
    }
  ...
  /**
   * Convenience: fetch first row with params
   */
  function fetchFirstRowParams($sql, $params = array())
  {
    $result = $this->exec_query_params($sql, $params);
    $row = null;
    if ($result)
    {
  ...
  /**
   * Convenience: fetch rows into map with params.
   */
  function fetchRowsIntoMapParams($sql, $column, $params = array(), $cumulative = 0, $limit = -1, $col2implode = '')
  {
    $items = null;
    $result = $this->exec_query_params($sql, $params, $limit);
    if ($result)
    {
  ...
  /**
   * Convenience: fetch column values into array with params.
   */
  function fetchColumnsIntoArrayParams($sql, $column, $params = array(), $limit = -1)
  {
    $items = null;
    $result = $this->exec_query_params($sql, $params, $limit);
    if ($result)
  ...
  /**
   * Safely escape a user string for usage inside a LIKE pattern. Escapes %, _, and the escape char itself.
   * Note: Use with "... LIKE ? ESCAPE '\\'" and bind the returned value.
   *
   * @param string $s raw input
   * @param string $escapeChar default '\\'
   * @return string escaped string (without surrounding %)
   */
  function likeEscape($s, $escapeChar = '\\')
  {
    if ($s === null) { return ''; }
    $esc = $escapeChar;
    $s = str_replace($esc, $esc.$esc, $s);
    $s = str_replace('%',  $esc.'%',  $s);
    $s = str_replace('_',  $esc.'_',  $s);
    return $s;
  }

  /** Build LIKE parameter for contains match: wraps with %...% after escaping */
  function likeParamContains($s, $escapeChar = '\\')
  {
    return '%' . $this->likeEscape($s, $escapeChar) . '%';
  }

  /** Build LIKE parameter for starts-with match: wraps with ...% after escaping */
  function likeParamStartsWith($s, $escapeChar = '\\')
  {
    return $this->likeEscape($s, $escapeChar) . '%';
  }

  /** Build LIKE parameter for ends-with match: wraps with %... after escaping */
  function likeParamEndsWith($s, $escapeChar = '\\')
  {
    return '%' . $this->likeEscape($s, $escapeChar);
  }
  ...
  /**
   * Whitelist and normalize ORDER BY column.
   * Accepts either an array of allowed column names (strings) or an associative map of
   * client-facing keys to actual SQL identifiers. Returns the safe identifier to inject
   * in SQL. If the candidate is not permitted, returns the provided default.
   *
   * Example usage:
   *   $col = $db->orderByColumn('name', ['name' => 'NH.name', 'id' => 'NH.id'], 'NH.id');
   *   $dir = $db->orderByDirection($_GET['dir'] ?? '', 'ASC');
   *   $sql .= " ORDER BY $col $dir";
   */
  function orderByColumn($candidate, $allowed, $default)
  {
    if (empty($allowed)) { return $default; }

    // If allowed is a simple list, treat values as both keys and values
    if (array_values($allowed) === $allowed) {
      $map = array();
      foreach ($allowed as $v) { $map[$v] = $v; }
    } else {
      $map = $allowed;
    }

    if (is_string($candidate)) {
      // exact match on client key
      if (isset($map[$candidate])) {
        return $map[$candidate];
      }

      // defensive: also allow direct match to a value if it exists in map values
  ...
  /**
   * Whitelist and normalize ORDER BY direction. Only ASC or DESC are returned.
   * Defaults to the provided $default ('ASC' recommended) if invalid.
   */
  function orderByDirection($candidate, $default = 'ASC')
  {
    $c = strtoupper(trim((string)$candidate));
    if ($c === 'ASC' || $c === 'DESC') { return $c; }
