...
if (!defined('REDMINE_SERIALIZATION_FIX')) {
    ...
    class redminerestInterface extends issueTrackerInterface {
        ...
        function setCfg($xmlString) {
            ...
            // Store configuration
            $this->apiKey = isset($config['apikey']) ? $config['apikey'] : 'c16548f2503932a9ef6d6d8f9a59393436e67f39';
    ...
    function addIssue($summary, $description) {
        ...
        try {
            ...
            // Add more details to the description
                ...
                // Format the description according to the specified format with actual extracted values
                // Use actual line breaks that Redmine will properly interpret
                // $formattedDescription = "BODY" . PHP_EOL . "... (Tester Notes + issue details)" . PHP_EOL . "--------------------------------" . PHP_EOL;
                $formattedDescription = " " . PHP_EOL;
                redmine_log("Formatted description::::::::::::::::::::: " . $formattedDescription);
                
                // Extract actual values from the available data instead of using N/A
                $scenarioId = $cleanSummary; // Use the formatted test case name as scenario ID
                $subScenario = "Test Case Execution"; // Default sub-scenario
                $testData = "Test execution data from TestLink"; // Default test data description
                $expectedResults = "Test should pass according to test case specifications"; // Default expected results
                $notes = $description; // Use the actual description passed to the function
                
                // Try to extract more specific information from the summary if available
                if (preg_match('/TC-(\d+):(.+?)(?:\s*-\s*Executed\s+ON.*)?$/', $summary, $matches)) {
                    $testCaseId = $matches[1];
                    $testCaseName = trim($matches[2]);
                    $scenarioId = "TC-" . $testCaseId . ": " . $testCaseName;
                    $subScenario = "Test Case ID: TC-" . $testCaseId;
                }
                
                // Extract structured information from description if it contains TestLink test data
                if (is_string($description) && !empty($description)) {
                    redmine_log("Processing description for data extraction: " . substr($description, 0, 200) . "...");
                    
                    // Extract Function ID
                    if (preg_match('/Function ID:\s*([^\r\n]+)/i', $description, $matches)) {
                        $functionId = trim($matches[1]);
                        $scenarioId = "Function ID: " . $functionId;
                        redmine_log("Extracted Function ID: " . $functionId);
                    }
                    
                    // Extract Action
                    if (preg_match('/Action:\s*([^\r\n]+)/i', $description, $matches)) {
                        $action = trim($matches[1]);
                        $subScenario = "Action: " . $action;
                    ...
                    if (preg_match('/Test scenario:\s*([^\r\n]+)/i', $description, $matches)) {
                        ...
                        $testData = "Test Scenario: " . $testScenario;
                    ...
                    // Extract Test result or any failure information
                    if (preg_match('/Test result:\s*\[(.*?)\]/s', $description, $matches)) {
                        $testResult = trim($matches[1]);
                        if (!empty($testResult)) {
                            $notes = "Test Result: " . $testResult;
                        } else {
                            $notes = "Test execution completed - see TestLink for detailed results";
                        }
                        redmine_log("Extracted Test Result: " . $testResult);
                    }
                    
                    // If no structured data found, use the full description
                    if ($testData === "Test execution data from TestLink" && strlen($description) > 50) {
                        $testData = substr($description, 0, 200) . "...";
                    }
                } else {
                    // If no description provided, use summary information
                    $notes = "Issue reported for test case: " . $cleanSummary;
                }
                
                // Append the template format for issue description
                $formattedDescription .= PHP_EOL ;
                $formattedDescription .= "Test Scenario/Function ID:: " . $scenarioId . PHP_EOL;
                $formattedDescription .= "Action ID/Sub-scenario:: " . $subScenario . PHP_EOL;
                $formattedDescription .= "Test Details:: " . $testData . PHP_EOL;
                $formattedDescription .= "Expected Test Results:: " . $expectedResults . PHP_EOL;
                $formattedDescription .= "Error Detail:: " . $notes . PHP_EOL . PHP_EOL;
