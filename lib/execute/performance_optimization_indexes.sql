...
CREATE INDEX IF NOT EXISTS idx_testplan_tcversions_tplan_build 
ON testplan_tcversions(testplan_id, tcversion_id);

CREATE INDEX IF NOT EXISTS idx_testplan_tcversions_composite 
ON testplan_tcversions(testplan_id, tcversion_id, platform_id);

-- Index for executions table (critical for execution status)
CREATE INDEX IF NOT EXISTS idx_executions_tcversion_build_platform 
ON executions(tcversion_id, build_id, platform_id, status);

CREATE INDEX IF NOT EXISTS idx_executions_status_timestamp 
ON executions(status, execution_ts DESC);

CREATE INDEX IF NOT EXISTS idx_executions_tcversion_latest 
ON executions(tcversion_id, execution_ts DESC);
...
CREATE INDEX IF NOT EXISTS idx_nodes_hierarchy_parent_type 
ON nodes_hierarchy(parent_id, node_type_id);

CREATE INDEX IF NOT EXISTS idx_nodes_hierarchy_name_search 
ON nodes_hierarchy(name);

-- Composite index for test case lookups
CREATE INDEX IF NOT EXISTS idx_nodes_hierarchy_testcase_lookup 
ON nodes_hierarchy(node_type_id, parent_id, name);
