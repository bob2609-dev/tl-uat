  ...
  if ($args->doSave || $args->doNavigate || $args->saveStepsPartialExec) {
    ...
    if( $args->save_results || $args->do_bulk_save) {  
      ...
      if (isset($args->use_custom_integration) && $args->use_custom_integration) {
        $_REQUEST['bug_summary'] = $args->bug_summary;
        require_once('custom_issue_handler.php');
        list($execSet,$gui->addIssueOp,$gui->uploadOp) = 
          custom_write_execution($db,$args,$_REQUEST,$its);
      } else {
        list($execSet,$gui->addIssueOp,$gui->uploadOp) = 
...
function init_args(&$dbHandler,$cfgObj) {
  ...
  $args->selected_integration_id = isset($_REQUEST['selected_integration_id']) ? intval($_REQUEST['selected_integration_id']) : 0;
 ...
 $args->doNavigate =  $args->doMoveNext || $args->doMovePrevious;
  ...
  if( ($args->issue_tracker_enabled = $info['issue_tracker_enabled']) ) {
    $requestedIntegrationID = ($args->selected_integration_id > 0) ? $args->selected_integration_id : null;
    $customIntegration = getCustomIntegrationForProject($dbHandler, $args->tproject_id, $requestedIntegrationID);
    if ($requestedIntegrationID && !$customIntegration) {
    ...
    if ($customIntegration) {
      ...
      $args->itsCfg = array(
        'issuetracker_name' => $customIntegration['name'],
        'issuetracker_type' => $customIntegration['type']
      );
      $bug_summary['maxLengh'] = 200;
    } else {
                              ...
                              $tplan_id, $tproject_id, $bugInterfaceOn, $bugInterface)
{
  ...
  foreach($other_execs as $tcversion_id => $execInfo) {
    ...
    for($idx = 0;$idx < $num_elem;$idx++) {
      ...
      if($bugInterfaceOn) {
        if(!is_null($bugInterface)) {
          $the_bugs = get_bugs_for_exec($db,$bugInterface,$exec_id);
          if(count($the_bugs) > 0) {
            $bugs[$exec_id] = $the_bugs;
          }
        } else {
          // Custom integration mode: $bugInterface is null.
          // Query execution_bugs directly to build a minimal bugs map
          // so the template renders the bug row (JS fetches Redmine details).
          $custom_bug_sql = "SELECT bug_id, tcstep_id FROM " . DB_TABLE_PREFIX . 
                            "execution_bugs WHERE execution_id = " . intval($exec_id);
          $custom_bug_result = $db->exec_query($custom_bug_sql);
          if($custom_bug_result) {
            $custom_bugs = array();
            while($bug_row = $db->fetch_array($custom_bug_result)) {
              $custom_bugs[$bug_row['bug_id']] = array(
                'bug_id'      => $bug_row['bug_id'],
                'tcstep_id'   => $bug_row['tcstep_id'],
                'step_number' => '',
                'build_name'  => '',
                'bug_url'     => '',
              );
            }
            if(count($custom_bugs) > 0) {
              $bugs[$exec_id] = $custom_bugs;
            }
          }
        }
      }
...
function initializeGui(&$dbHandler,&$argsObj,&$cfgObj,&$tplanMgr,&$tcaseMgr,&$issueTracker,&$codeTracker) 
{
  ...
  if (isset($argsObj->use_custom_integration) && $argsObj->use_custom_integration &&
      isset($argsObj->custom_integration) && is_array($argsObj->custom_integration)) {
    $customIntegration = $argsObj->custom_integration;
...
function initWebEditors(&$guiObj,$cfgObj,$baseHREF) {
  if( $guiObj->can_use_bulk_op ) {
      ...
      if (!is_array($guiObj->exec_notes_editors)) {
          $guiObj->exec_notes_editors = array();
      }
  }
}
