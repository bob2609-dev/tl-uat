...
BEGIN
  SELECT
      ...
      (
        ...
        LEFT JOIN node_hierarchy_paths_v4 nhp_total ON parent_nh_total.id = nhp_total.node_id
        ...
        WHERE nhp_total.full_path = nhp.full_path
          AND ((p_project_id   IS NULL OR p_project_id   = 0) OR tproj_total.id = p_project_id)
          AND ((p_testplan_id  IS NULL OR p_testplan_id  = 0) OR tp_total.id    = p_testplan_id)
      ) AS total_testcases,
  ...
  LEFT JOIN node_hierarchy_paths_v4 nhp ON parent_nh.id = nhp.node_id
  ...
  WHERE 1=1
    AND nhp.full_path IS NOT NULL AND nhp.full_path <> ''
    ...
    AND ((p_execution_path IS NULL OR p_execution_path = '') OR nhp.full_path LIKE CONCAT('%', p_execution_path, '%'))
  ...
  GROUP BY nhp.full_path, tptcv.testplan_id
