...
function checkAccessToExec(&$dbH,&$ct) {
    ...
    if (!isset($_SESSION['currentUser'])) {
        echo "<!-- DEBUG: No currentUser in session -->\n";
        return array('testplan_execute' => false, 'exec_ro_access' => false);
    }
    
    $user = $_SESSION['currentUser'];
    echo "<!-- DEBUG: User ID: " . $user->dbID . ", Role: " . $user->globalRoleID . ", Name: " . $user->login . " -->\n";
    echo "<!-- DEBUG: Test Plan ID: {$tplan_id}, Test Project ID: {$tproject_id} -->\n";
    
    $grants = null;
    $k2a = array('testplan_execute','exec_ro_access');
    foreach($k2a as $r2c) {
        $grants[$r2c] = false;
        echo "<!-- DEBUG: Checking right {$r2c}... -->\n";
        if( $user->hasRight($dbH,$r2c,$tproject_id,$tplan_id,true) || $user->globalRoleID == TL_ROLES_ADMIN ) {
            $grants[$r2c] = true;
            echo "<!-- DEBUG: Right {$r2c} GRANTED -->\n";
        } else {
            echo "<!-- DEBUG: Right {$r2c} DENIED -->\n";
        }    
    }  
    echo "<!-- DEBUG: Final grants: " . json_encode($grants) . " -->\n";
