#!/usr/bin/env python3
"""
Create standalone executable for TestLink Optimized Execution Module
Simple approach: bundle everything into one executable
"""
...
# ── Embedded file contents (defined at module level so f-strings stay simple) ──

EMBEDDED_ENV = """\
# TestLink Optimized Execution Module - Production Configuration
...
# Uses {placeholders} filled by .format(); literal braces in Python code are
# doubled {{ }} so they survive .format() safely.
...
def create_standalone():
    ...
    print("Creating TestLink Optimized Execution Module - Standalone")
    print("=" * 60)

    output_file = Path("TestLinkOptimizedExecution.py")

    # Fill the template (double-braces in template become single braces here)
    launcher_code = LAUNCHER_TEMPLATE.format(
        env=EMBEDDED_ENV,
        main=EMBEDDED_MAIN,
        config=EMBEDDED_CONFIG,
        database=EMBEDDED_DATABASE,
        requirements=EMBEDDED_REQUIREMENTS,
    )

    output_file.write_text(launcher_code, encoding="utf-8")
    output_file.chmod(0o755)

    print(f"Created: {output_file}")
    print("Usage:   python TestLinkOptimizedExecution.py")
    return 0


if __name__ == "__main__":
    sys.exit(create_standalone())