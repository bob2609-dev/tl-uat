...
BEGIN
    ...
    -- CTE for user assignments with historical date filtering
    WITH user_assignments_historical AS (
        SELECT 
            ...
            tptc.tcversion_id,
            tptc.platform_id,
            0 as build_id  -- Default build_id for assignments
    ...
    test_cases_historical_status AS (
        SELECT 
            ...
            uah.tcversion_id,
            uah.platform_id,
            COALESCE(leh.status, 'n') AS execution_status,  -- Default to 'n' (not run) if no execution in range
            leh.execution_ts,
        ...
        WHERE (v_date_from IS NULL OR v_date_to IS NULL OR leh.id IS NOT NULL OR uah.user_id IS NOT NULL)
    )
