...
def main():
    ...
    try:
        ...
        try:
            ...
            hierarchical_data = data_processor.get_test_execution_data(filters)
            ...
            if hierarchical_data:
                ...
                if 'metrics_by_suite' in hierarchical_data:
                    ...
                    for suite_id, suite_metrics in hierarchical_data['metrics_by_suite'].items():
                        ...
                        # Add count key if missing
                        if 'count' not in suite_metrics:
                            logging.warning(f"Suite {suite_id} is missing 'count' key, adding default")
                            
                            # Try to use testcase_count if available
                            if 'testcase_count' in suite_metrics:
                                suite_metrics['count'] = suite_metrics['testcase_count']
                                logging.info(f"Using testcase_count ({suite_metrics['testcase_count']}) for count in suite {suite_id}")
                            else:
                                suite_metrics['count'] = 0
                                logging.warning(f"Added default count=0 for suite {suite_id}")
                        
                        # Check for paths
                        if 'full_path' in suite_metrics:
                            logging.debug(f"Suite {suite_id} has path: {suite_metrics['full_path'][:100]}...")
                        else:
                            logging.warning(f"Suite {suite_id} is missing 'full_path'")
                else:
                    logging.error("Missing metrics_by_suite in hierarchical_data")
            else:
                logging.warning("No data returned from data processor")
        ...
        try:
            ...
            if not args.output:
                timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
                output_file = os.path.join(output_dir, f"testlink_export_{timestamp}.xlsx")
            else:
            ...
            excel_exporter.export_data(hierarchical_data)
