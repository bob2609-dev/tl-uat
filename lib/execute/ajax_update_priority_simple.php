<?php
/**
 * Minimal test - check each step individually with file logging
 */
...
try {
    ...
    try {
        ...
        // Log all database-related constants
        $db_constants = array();
        ...
        logToFile("Database constants found: " . print_r($db_constants, true));
        
        $response['debug']['config_vars'] = array(
            'DB_TABLE_PREFIX' => defined('DB_TABLE_PREFIX') ? DB_TABLE_PREFIX : 'NOT_DEFINED',
            'TL_VERSION' => defined('TL_VERSION') ? TL_VERSION : 'NOT_DEFINED',
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        logToFile("Step 4: POST data received - tcvID=$tcvID, priority=$priority");
        ...
        if ($tcvID > 0 && !empty($priority)) {
            ...
            logToFile("Invalid parameters: tcvID=$tcvID, priority='$priority'");
        }
    } else {
