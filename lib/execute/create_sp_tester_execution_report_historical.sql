-- Create stored procedure for historical tester execution report
-- This procedure provides accurate point-in-time data for historical date filtering
...
-- Create helper procedure for getting summary data
DELIMITER $$

CREATE PROCEDURE sp_tester_execution_report_historical_summary(
    IN p_testproject_id INT,
    IN p_testplan_id INT, 
    IN p_build_id INT,
    IN p_tester_id INT,
...
BEGIN
    -- Call the main procedure and get summary totals
    SELECT 
        ...
        SUM(not_run) AS total_not_run,
        SUM(skipped) AS total_skipped,
        SUM(warning) AS total_warning,
        ROUND(
            CASE 
                WHEN SUM(passed) + SUM(failed) > 0 
                THEN (SUM(passed) / (SUM(passed) + SUM(failed))) * 100 
                ELSE 0 
            END, 1
        ) AS overall_pass_rate
    FROM (
        CALL sp_tester_execution_report_historical(
            p_testproject_id, p_testplan_id, p_build_id, p_tester_id, 
...
-- Grant execute permissions (adjust user/role as needed)
-- GRANT EXECUTE ON PROCEDURE sp_tester_execution_report_historical TO 'testlink_user'@'%';
-- GRANT EXECUTE ON PROCEDURE sp_tester_execution_report_historical_summary TO 'testlink_user'@'%';

-- Example usage:
-- CALL sp_tester_execution_report_historical(1, NULL, NULL, NULL, 'assigned', '2024-01-01', '2024-01-31');
-- CALL sp_tester_execution_report_historical_summary(1, NULL, NULL, NULL, 'assigned', '2024-01-01', '2024-01-31');
