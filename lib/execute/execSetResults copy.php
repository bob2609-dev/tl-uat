...
if(!is_null($linked_tcversions)) {
  ...
  if ($args->doSave || $args->doNavigate || $args->saveStepsPartialExec) {
    ...
    if( $args->save_results || $args->do_bulk_save) {  
      ...
      // Performance optimization: Update tree node without full refresh
      echo '<script type="text/javascript">';
      echo 'if (typeof window.parent.updateTestCaseNodeInTree === "function") {';
      echo '  window.parent.updateTestCaseNodeInTree(' . $tcase_id . ', \'' . $args->statusSingle[$args->version_id] . '\', \'' . getStatusColor($args->statusSingle[$args->version_id]) . '\');';
      echo '}';
      echo '</script>';
    }
    
    // Get status color mapping for tree update
    function getStatusColor($status) {
      $statusColors = array(
        'p' => '#008000',  // Passed - Green
        'f' => '#FF0000',  // Failed - Red  
        'b' => '#FFA500',  // Blocked - Orange/Red
        'n' => '#808080',  // Not Run - Gray
        's' => '#FFD700',  // Skipped - Yellow
        'w' => '#0000FF',  // Warning - Blue
        'i' => '#800080'  // Invalid - Dark Gray
      );
      return isset($statusColors[$status]) ? $statusColors[$status] : '#000000';
    } 
  }
