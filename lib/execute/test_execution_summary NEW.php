...
FROM 
    ...
    -- Find path for each test suite
    JOIN node_hierarchy_paths_v2 nhp ON nhp.node_id = suite.parent_id
GROUP BY 
    nhp.full_path 
ORDER BY 
    nhp.full_path";
...
foreach ($pathDetails as &$path) {
    ...
    if (($path['passed_count'] + $path['failed_count']) > 0) {
        $path['pass_rate'] = round(($path['passed_count'] / ($path['passed_count'] + $path['failed_count'])) * 100, 2);
    } else {
        $path['pass_rate'] = 0;
    }
    
    // Calculate fail rate = failed / (passed + failed)
    if (($path['passed_count'] + $path['failed_count']) > 0) {
        $path['fail_rate'] = round(($path['failed_count'] / ($path['passed_count'] + $path['failed_count'])) * 100, 2);
    } else {
        $path['fail_rate'] = 0;
    }
    ...
    // Calculate pending rate = not run / (total - blocked)
    if (($path['testcase_count'] - $path['blocked_count']) > 0) {
        $path['pending_rate'] = round(($path['not_run_count'] / ($path['testcase_count'] - $path['blocked_count'])) * 100, 2);
    } else {
        $path['pending_rate'] = 0;
    }
}
