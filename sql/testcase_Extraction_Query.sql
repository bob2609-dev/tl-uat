...
-- Column order matches C2B1_CCL_CLEARING.xlsx exactly, with Last Execution Date as final column
use testlink_db;
SELECT 
    ...
    (SELECT COUNT(*) FROM executions e WHERE e.tcversion_id = nh_tcv.id) AS execution_count,
    (SELECT COUNT(*) FROM executions e2 WHERE e2.tcversion_id = nh_tcv.id AND e2.execution_ts IS NOT NULL) AS non_null_execution_count,
    exec.execution_id,
    exec.status AS last_execution_status,
    first_exec.first_execution_ts AS first_exec_debug,
    exec.execution_ts AS last_exec_debug,
    (SELECT MAX(execution_ts) FROM executions e3 WHERE e3.tcversion_id = nh_tcv.id) AS max_exec_ts,
    (SELECT MIN(execution_ts) FROM executions e4 WHERE e4.tcversion_id = nh_tcv.id) AS min_exec_ts,
    
    -- 12. First Execution Date
    first_exec.first_execution_ts AS 'First Execution Date',
    
    -- 13. Last Execution Date
...
--     ,
    ...
    -- Additional useful fields for reference (commented out to keep output clean)
    ...
    -- Execution Status and other execution details (commented out to keep output clean)
    CASE exec.status
        WHEN 'p' THEN 'Passed'
        WHEN 'f' THEN 'Failed'
        WHEN 'b' THEN 'Blocked'
...
--     cev.value
--     ,
    -- exec.execution_duration AS 'Execution Duration (min)',
    -- exec.notes AS 'Execution Notes'
...
FROM 
    ...
    -- Debug: Check if there are any executions at all
    -- SELECT COUNT(*) FROM executions WHERE tcversion_id IN (SELECT id FROM nodes_hierarchy WHERE node_type_id = 4)
    ...
    LEFT JOIN (
        ...
        FROM executions
        WHERE execution_ts IS NOT NULL  -- Only consider executions with timestamps
...
WHERE 
    ...
    AND nh_tc.parent_id in (157430,190446,167862,148888,163352,190448,190449,193017,216448,192626,170525,151013,151869,151870,156063,168489,168752,168753,168754,168798,168799,168800,190447,169198,154356)  -- Replace with your test suite ID
--     
--     106600
-- 193058
-- 193059
...
--     AND exec.execution_ts >= '2025-07-22'  -- Test cases executed on or after specific date
--     AND exec.execution_ts <  '2025-07-22' -- Test cases executed on or before specific date
...
--    AND exec.status = 'b'  -- Only passed test cases (p=Passed, f=Failed, b=Blocked, n=Not Run, s=Skipped, w=Wrong)
--     AND exec.status IN ('p','f','b')  -- Only passed test cases (p=Passed, f=Failed, b=Blocked, n=Not Run, s=Skipped, w=Wrong)
...
--     AND exec.execution_ts IS NOT NULL  -- Only test cases that have been executed
    ...
    -- Filter for test cases with either first or last execution date (comment out if not needed)
    -- AND (first_exec.first_execution_ts IS NOT NULL OR exec.execution_ts IS NOT NULL)

ORDER BY 
    first_exec.first_execution_ts,  -- Order by first execution date
    exec.execution_ts DESC,        -- Then by most recent execution
